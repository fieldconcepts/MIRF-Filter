...
class MirfFile:
    ...
    def open_file(self, filename):
        ...
        with open(filename,'rb') as f:
            ...
            self._decode_channels() #decodes binary channel data and stores in an array 'self.channels'
            self._decode_data(f)
    ...
    def save_file(self,filename):
        ...
        if os.path.exists(filename):
            raise Exception("Cannot overwrite MIRF files!")
            
        with open(filename, 'wb') as f:
            self._write_header() # Use variables to create binary header
            f.write(self._raw_header)
            
            self._write_channels() # Use variables to create binary channel headers
            f.write(self._channel_headers)
            
            data = self.get_all_data().astype('float32').tobytes('F')
            f.write(data)
...
#             print("{:02d}: {}".format(i,d))
        ...
        self.time = datetime.datetime(fields[9],fields[10],fields[11],fields[12],fields[13],fields[14],tzinfo=TZ())
    ...
    def get_all_data(self):
        "Returns the data in all channels as a NumPy datablock."
        ret = []
        for ch in self.channels:
            if ch.N:
                ret.append(ch.data)
    ...
    def _decode_channels(self):
        self.channels = [MirfFile._Channel(i+1,self._channel_headers[x:x + 0x40]) for i,x in enumerate(range(0, len(self._channel_headers), 0x40))]
